		...
		while($j1<count($data1))		
		{
			...
			$curTime = (($curTime[0] * 60) + $curTime[1]);
				...
				$pos2=calLong($data3[1]);
					...
					'" sessionID="'.$sessionID.'"  fuel="3" extraInfo="'.$extraInfo.'"  route="'.$rtName.'"/>';
...
function storeData($data,$km,$devName,$sTime2,$eTime2)
{
  ...
  '" sessionID="'.$sessionID.'" fuel=3 extraInfo="'.$extraInfo.'"  deviceModel="'.$devModel.'" route="'.$rtName.'" other="'.$data3[7].'"/>';
