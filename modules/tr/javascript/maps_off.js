...
function updateMarker(i, length, point, markers, speed, direction, gpsTime, geodate, deviceName,
	...
	{					  
		...
		var marker = new ELabel(map.getCenter(), 
    ...
    GEvent.addListener(marker, "click", function() {
		...
		geocoder.getLocations((latitude+","+longitude), function(response)																																						    	{
	  	...
	  	var ptText='<tr><td colspan=2 align=center><input type=text name="txtMapPoint" id="txtMapPoint" value="GeofenceName" />&nbsp;&nbsp;<a href=# onclick="addThisPoint('+latitude+','+longitude+');">&nbsp;<strong class=input_submit>Set</strong></a></td></tr>';
        ...
        //+ str		
        + "<tr><td align=left><b>Speed:</b>:" + speed +  " Kmph&nbsp;</td></tr>"
...
function createAllMarker(i, length, point, speed, direction, gpsTime, geodate, deviceName,
	...
	geocoder.getLocations((latitude+","+longitude), function(response)																																						    {
	  ...
	  var ptText= '<tr><td colspan=2 align=left><input type=text name="txtMapPoint" id="txtMapPoint" value="Your Geofence Name" />&nbsp;&nbsp;<a href=# onclick="addThisPoint('+latitude+','+longitude+');">&nbsp;<strong class=input_submit>Set</strong></a></td></tr>';
...
function showMapOnDate(sDate,sTime,eDate,eTime,devId) {
	...
	if(diftd <= 6 && diftd >= 0)
	{
		...
		GDownloadUrl(url, loadGPSLocations);
		document.frmMapData.cmdPlay.disabled = false;
	}
	else if(diftd > 6)
	{
...
{
	if(playBtnObj.value == "Play")
	{
		playBtnObj.value = "Pause";
		showWait('Point Animation Started ...');
		anim();
	}
	else
	{
		playBtnObj.value = "Play";
		showWait('Point Animation Paused ...');
		haltAnim();
	}
}
function haltAnim() 
...
function anim() 
{
	...
	if(count < myMarker.length) 
	{
		map.panTo(new GLatLng(parseFloat(myMarker[count].getAttribute("latitude")), parseFloat(myMarker[count].getAttribute("longitude"))));
		var delay = 3400;
		GEvent.trigger(myGMarker[count], "click");
	 
	 	if(count >0)
		{
			pits1 =  new GLatLng(parseFloat(myMarker[(count-1)].getAttribute("latitude")),parseFloat(myMarker[(count-1)].getAttribute("longitude")));
			pits2 =  new GLatLng(parseFloat(myMarker[count].getAttribute("latitude")),parseFloat(myMarker[count].getAttribute("longitude")));
...
var eng1,eng2,eng3,ac1,ac2,ac3,ig1,ig2,ig3,sos1,sos2,sos3;
eng3=ac3=ig3=sos3 = 'No Information';
for(i = 0; i < mySplitResult.length; i++){
  eng1 =mySplitResult[i].substr(0,3);
	if(eng1=='[1='){

	  eng2= mySplitResult[i].substr(3).trim();
	  if(eng2=='1'){
		  eng3 = 'On';
	  } else {
         eng3 = 'Off';
	   ...
	   ac1 =mySplitResult[i].substr(0,4).trim();
	if(ac1=='[2='){
	  ac2= mySplitResult[i].substr(4);
	   if(ac2=='1'){
		  ac3 = 'On';
	  } else {
         ac3 = 'Off';
	 ...
	 ig1 =mySplitResult[i].substr(0,4).trim();

	if(ig1=='[3='){
	  ig2= mySplitResult[i].substr(4);
    if(ig2=='1'){
		  ig3 = 'On';
	  } else {
         ig3 = 'Off';
	 ...
	 sos1 =mySplitResult[i].substr(0,4).trim();

	if(sos1=='[4='){
	  sos2= mySplitResult[i].substr(4);
	  if(sos2=='1'){
		  sos3 = 'On';
	  } else {
         sos3 = 'Off';
    ...
    document.getElementById("Engine").innerHTML='Engine&nbsp;:&nbsp;'+eng3;
	document.getElementById("AC").innerHTML='A/C&nbsp;:&nbsp;'+ac3;
	document.getElementById("Ignition").innerHTML='Ignition&nbsp;:&nbsp;'+ig3;
	document.getElementById("Sos").innerHTML='Sos&nbsp;:&nbsp;'+sos3;
...
function createMarker(i, length, point, markers, speed, direction, gpsTime, geodate, deviceName,deviceIMEI, distance,sessionID, accuracy, isLocationValid, extraInfo,altitute,latitude,longitude) 
{
    var icon = new GIcon();
