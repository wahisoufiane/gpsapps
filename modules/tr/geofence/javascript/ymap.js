...
function loadYRoutes(data, responseCode) {
    if (data.length == 0) {
        ...
        for (i = 0; i < routes.length; i++) {
			...
			if(routes[i].getAttribute("phoneNumber")==routes[i].getAttribute("select"))
			{
				...
				//document.write(url);
			    //GDownloadUrl(url, loadYGPSLocations);
				
				ajax1.requestFile = url;
				ajax1.onCompletion = function(){loadYGPSLocations()};
				ajax1.runAJAX();
			}
...
function changeInterval_offline(val) {
	...
	//alert(interval);
	   loadYGPSLocations(data1, responseCode1)
	   }
	else {
...
function loadYGPSLocations() {
    ...
    if (data.length == 0) {
		...
		// create list of GPS data locations from our XML
			...
			map.drawZoomAndCenter(new YGeoPoint(parseFloat(markers[length-1].getAttribute("latitude")),parseFloat(markers[length-1].getAttribute("longitude"))),zoomLevel);
...
function callDistance(map,markers)
{
	...
	for (var p1 = 0; p1 < (length1-1); p1++) 
	{
		...
		if((t3!=parseFloat(markers[p1].getAttribute("latitude"))) && (t4!=parseFloat(markers[p1].getAttribute("longitude"))))
		{
			if(p1==0)
			{
				...
				pts[t] = new YGeoPoint(parseFloat(markers[0].getAttribute("latitude")),parseFloat(markers[0].getAttribute("longitude")));
				t++;
...
//					map.addOverlay(marker6);
			...
			if(tmp!=(Math.round(dis/1000)))
			{
				if(tmp>=1)
				{
					callYFunction(p1,map,markers,length1);
					pts[t] = new YGeoPoint(parseFloat(markers[p1].getAttribute("latitude")),parseFloat(markers[p1].getAttribute("longitude")));
					t++;
				}
				tmp=Math.round(dis/1000);
				
			}
			t3=parseFloat(markers[p1].getAttribute("latitude"));
			t4=parseFloat(markers[p1].getAttribute("longitude"));	
...
function callYFunction(p,map,markers,length)
{
	var point = new YGeoPoint(parseFloat(markers[p].getAttribute("latitude")),parseFloat(markers[p].getAttribute("longitude")));
	
	d1=dateTimeConvert(markers[p].getAttribute("gpsTime"));	
	//d1=eval(markers[i].getAttribute("gpsTime")+"0000-00-00 05:30:00");
	var marker = createYMarker(p, length, point,
...
function createYMarker(i, length, point, speed, direction, locationMethod, gpsTime,
...
function deleteRouteResponse(data, responseCode) {
    ...
    GDownloadUrl('getroutes.php', loadYRoutes);
}
