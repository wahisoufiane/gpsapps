...
GeometryControls.prototype.bindInfoWindow = function(geomInfo){
  ...
  var geoidInput = get$(cssId + "-geoid");
  ...
  geoidInput.value = (geoid[1]) ? ((geoid[0] === geoid[1]) ? geoid[0] : geoid[1]) : ""; //return the saved data or most recent edit
  ...
  GEvent.addDomListener(geoidInput,"change", function(){
    geoid[1] = geoidInput.value;
  });
  ...
  GEvent.addDomListener(get$(cssId + "-delete"),"click",function(){
    if(confirm("Are you sure you want to delete this?")){
	  ...
	  ajax1.requestFile = '../ajax_server.php?deleteGeoId='+geoid[0];
...
GeometryControls.prototype.handleKmlDataResponse_ = function(data, responseCode){
  ...
  if (responseCode == 200) {
    try{
	  ...
	  var side_bar = '<ul id="menu"><li><a href="#">Geofence Points</a><ul>';
	  
      for (var i = 0; i < placemarks.length; i++) {
        var name = EGeoXml.value(placemarks[i].getElementsByTagName("name")[0]);		
		var geoid = EGeoXml.value(placemarks[i].getElementsByTagName("geoid")[0]);
		
		side_bar += "<li><a href='#' class=green_link onclick='showThisGeoPoint("+geoid+");'>"+name + "</a></li>";
...
GeometryControls.prototype.saveData = function(opts){
  ...
  if(opts.allData === true){
	...
	recordJSON.geoid = record.geoid[0];
