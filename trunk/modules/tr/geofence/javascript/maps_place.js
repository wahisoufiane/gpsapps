...
function loadGPSLocations(data, responseCode) {
    ...
    if (data.length == 0) {
        ...
        if (GBrowserIsCompatible()) {
			...
			document.getElementById("distance").innerHTML=callDistance(map,markers)+" Km";
		...
		for (var i = 1; i < (length-1); i+=interval) 
		{
			if((t1!=parseFloat(markers[i].getAttribute("latitude"))) && (t2!=parseFloat(markers[i].getAttribute("longitude"))))
			{
					...
					t1=parseFloat(markers[i].getAttribute("latitude"));
					t2=parseFloat(markers[i].getAttribute("longitude"));
			}
				c1++;
		}
...
function callDistance(map,markers)
{
	...
	for (var p1 = 0; p1 < (length1-1); p1++) 
	{
		...
		if((t3!=parseFloat(markers[p1].getAttribute("latitude"))) && (t4!=parseFloat(markers[p1].getAttribute("longitude"))))
		{
			...
			if(tmp!=(Math.round(dis/10000)))
			{
				if(tmp>=1)
				{
					callFunction(p1,map,markers,length1);
				}
				tmp=Math.round(dis/10000);
...
function dateTimeConvert(str)
{
	...
	if((d4%60)>0)
	{
		var tem=parseFloat(d2[0])+(Math.floor((parseFloat(d2[1])+30)/60)+5);
		if(tem>12 && tem<24)
		{
			d5=(tem)-12;
			//alert(d5);
    ...
    if (hasMap()) {
        ...
        if (currentInterval > 0) { // currently running at an interval
            ...
            if (newInterval > 0) { // moving to another interval (3)
                ...
                intervalID = setInterval("getRouteForMap_offline('"+date+"');", newInterval * 1000);
...
function changeZoomLevel_offline(date_offline) {
    if (hasMap()) {
        zoomLevel = zoomLevelSelect.selectedIndex + 1;

        getRouteForMap_offline(date_offline);
