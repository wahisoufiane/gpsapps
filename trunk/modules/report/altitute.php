...
<tr><th colspan="4">Altitude Graph</th></tr>
...
$spdHighData =array();
$totalDistance=0;
$tripdata = "Date,Device ID,User ID,Start Point,Start Time,End Point,End Time,Duration,Avg. Altitute,Dist. Covered";
$tripdata .= "@";
$ct=0;
$inc = 5;
	//$file = $GLOBALS[dataPath]."src/data/".date("d-m-Y", strtotime($sdate))."/".$srcData[1].".txt";   
		 $file = $GLOBALS[dataPath]."gpsapp/data/".date("d-m-Y", strtotime($sdate))."/".$srcData[1].".txt";   
		 ...
		 while($j1 < count($data1))
		 {
				...
				if(count($data2)>1)
				{
					...
					if($curTime >= $strtTime && $curTime<=$endTime && $data3[3] > 0)
					{
						...
						{
							$altData [] = $data3[5];
							//echo date("H:i:s",strtotime($data3[9]))."<br>";
	...
	//echo $altData = implode(",",$altData);
	//echo $spdTime = implode(",",$spdTime);
	//print_r($spdTime);
	//echo count($altData);
	//exit;
	if(count($altData)>0)
...
<script type="text/javascript">
 ...
 $(document).ready(function() {
	...
	title: {
		text: 'Altitude Graph - <?php echo $srcData[0]." (".$_POST[from_date].")";?>'
	},
	xAxis: {
	...
	tooltip: {
		formatter: function() {
			return 'Time: '+ this.x +' Altitude: '+ this.y;
		}
	},
		series: [{
		type: 'area',
