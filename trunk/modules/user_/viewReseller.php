...
$sql = "SELECT * FROM tb_clientinfo WHERE ci_clientType='Reseller' AND ci_clientId=".$_SESSION[clientID]." ORDER BY ci_clientName ASC";
$rows = $db->query($sql);
...
<table class="gridform_final">
   ...
   if($db->affected_rows > 0)
   {
	...
	?>
        ...
        <td><!--<a href="#" onclick="funEditUser('<?php echo $record[ci_id];?>','?ch=addResellerSubscription')">Add</a></span> / --><a href="#" onclick="funEditUser('<?php echo $record[ci_id];?>','?ch=viewResellerSubscription')">view </a></span></td>
